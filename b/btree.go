...
func (p *btEpool) get(err error, hit bool, i int, k float64, q *d, t *Tree, ver int64) *Enumerator {
	x := p.Get().(*Enumerator)
...
func (q *x) insert(i int, k float64, ch interface{}) *x {
	c := q.c
	if i < c {
		q.x[c+1].ch = q.x[c].ch
...
// true.
func (t *Tree) Delete(k float64) (ok bool) {
	pi := -1
	var p *x
	q := t.r
	if q == nil {
...
func (t *Tree) extract(q *d, i int) { // (r uint) {
	t.ver++
...
func (t *Tree) find(q interface{}, k float64) (i int, ok bool) {
	var mk float64
	l := 0
	switch x := q.(type) {
	case *x:
...
func (t *Tree) First() (k float64, v uint) {
	if q := t.first; q != nil {
		q := &q.d[0]
		k, v = q.k, q.v
	}
...
func (t *Tree) Get(k float64) (v uint, ok bool) {
	q := t.r
	if q == nil {
		return
	}
...
func (t *Tree) insert(q *d, i int, k float64, v uint) *d {
	t.ver++
	c := q.c
	if i < c {
...
// (zero-value, zero-value) if the tree is empty.
func (t *Tree) Last() (k float64, v uint) {
	if q := t.last; q != nil {
		q := &q.d[q.c-1]
		k, v = q.k, q.v
	}
...
func (t *Tree) overflow(p *x, q *d, pi, i int, k float64, v uint) {
	t.ver++
	l, r := p.siblings(pi)
...
func (t *Tree) Seek(k float64) (e *Enumerator, ok bool) {
	q := t.r
	if q == nil {
...
func (t *Tree) Set(k float64, v uint) {
	//dbg("--- PRE Set(%v, %v)\n%s", k, v, t.dump())
	//defer func() {
	//	dbg("--- POST\n%s\n====\n", t.dump())
	//}()
...
//
// 	tree.Put(k, func(float64, bool){ return v, true })
//
// modulo the differing return values.
func (t *Tree) Put(k float64, upd func(oldV uint, exists bool) (newV uint, write bool)) (oldV uint, written bool) {
	pi := -1
	var p *x
	q := t.r
	var newV uint
...
func (t *Tree) split(p *x, q *d, pi, i int, k float64, v uint) {
	t.ver++
	r := btDPool.Get().(*d)
	if q.n != nil {
		r.n = q.n
...
// io.EOF is returned.
func (e *Enumerator) Next() (k float64, v uint, err error) {
...
// == io.EOF is returned.
func (e *Enumerator) Prev() (k float64, v uint, err error) {
	if err = e.err; err != nil {
		return
	}
